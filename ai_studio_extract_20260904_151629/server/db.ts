...
export function attachEphemeralApiKey(project: Project | null): Project | null {
  ...
  if (!project.reasoning_model_preferences) {
    const primaryModelId = project.reasoning_config?.model_id || project.ai_model || 'gemini-3.8-flash';
    project.reasoning_model_preferences = {
      ...
      fallback_pool: [
        ...
        { provider: 'google', model_id: 'gemini-3.1-pro-preview', priority: 3, display_name: 'Gemini 3.1 Pro' },
      ],
      force_model: false,
      stage_routing: {},
    };
...
// ---------------------------------------------------------------------------
// DB adapter (Firestore / JSON implementation)
// ---------------------------------------------------------------------------
export const firestoreDb = {
  ...
  async getCharacterContinuityStates(projectId: string): Promise<CharacterContinuityState[]> {
    ...
    const fsdb = getFirestore();
    const docData = await getDocData<any>(fsdb, 'continuity_states', projectId);
    let states: CharacterContinuityState[] = [];
    if (docData) {
      if (Array.isArray(docData)) {
        states = docData;
      } else if (docData.states && Array.isArray(docData.states)) {
  ...
  async getProviders(): Promise<AIProvider[]> {
    if (!USE_FIRESTORE) {
      return Object.values(jsonState.ai_providers || {});
    }
    const fsdb = getFirestore();
    const snap = await colRef(fsdb, 'ai_providers').get();
    return snap.docs.map((d: any) => ({ ...(d.data() as AIProvider), id: d.id }));
  },

  async getProvider(id: string): Promise<AIProvider | null> {
    if (!USE_FIRESTORE) {
      return jsonState.ai_providers?.[id] || null;
    }
    const fsdb = getFirestore();
    return getDocData<AIProvider>(fsdb, 'ai_providers', id);
  },

  async saveProvider(provider: AIProvider): Promise<AIProvider> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_providers) jsonState.ai_providers = {};
      jsonState.ai_providers[provider.id] = provider;
      saveJsonState(jsonState);
      return provider;
    }
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_providers', provider.id).set(sanitizeForFirestore(provider));
    return provider;
  },

  async deleteProvider(id: string): Promise<boolean> {
    if (!USE_FIRESTORE) {
      if (jsonState.ai_providers?.[id]) {
        delete jsonState.ai_providers[id];
        saveJsonState(jsonState);
        return true;
      }
      return false;
    ...
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_providers', id).delete();
    return true;
  },

  async getCredentials(): Promise<AICredential[]> {
    if (!USE_FIRESTORE) {
      return Object.values(jsonState.ai_credentials || {});
    }
    const fsdb = getFirestore();
    const snap = await colRef(fsdb, 'ai_credentials').get();
    return snap.docs.map((d: any) => ({ ...(d.data() as AICredential), id: d.id }));
  },

  async getCredential(id: string): Promise<AICredential | null> {
    if (!USE_FIRESTORE) {
      return jsonState.ai_credentials?.[id] || null;
    }
    const fsdb = getFirestore();
    return getDocData<AICredential>(fsdb, 'ai_credentials', id);
  },

  async saveCredential(cred: AICredential): Promise<AICredential> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_credentials) jsonState.ai_credentials = {};
      jsonState.ai_credentials[cred.id] = cred;
      saveJsonState(jsonState);
      return cred;
    }
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_credentials', cred.id).set(sanitizeForFirestore(cred));
    return cred;
  },

  async deleteCredential(id: string): Promise<boolean> {
    if (!USE_FIRESTORE) {
      if (jsonState.ai_credentials?.[id]) {
        delete jsonState.ai_credentials[id];
        saveJsonState(jsonState);
        return true;
      }
      return false;
    ...
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_credentials', id).delete();
    return true;
  },

  async getModels(): Promise<AIModel[]> {
    if (!USE_FIRESTORE) {
      return Object.values(jsonState.ai_models || {});
    }
    const fsdb = getFirestore();
    const snap = await colRef(fsdb, 'ai_models').get();
    return snap.docs.map((d: any) => ({ ...(d.data() as AIModel) }));
  },

  async getModel(id: string, providerId?: string): Promise<AIModel | null> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_models) return null;
      if (providerId) {
        return jsonState.ai_models[`${providerId}::${id}`] || jsonState.ai_models[id] || null;
      }
      if (jsonState.ai_models[id]) return jsonState.ai_models[id];
      const found = Object.values(jsonState.ai_models).find(m => m.id === id);
      return found || null;
    }
    const fsdb = getFirestore();
    if (providerId) {
      const doc = await getDocData<AIModel>(fsdb, 'ai_models', `${providerId}__${id}`);
      if (doc) return doc;
    }
    return getDocData<AIModel>(fsdb, 'ai_models', id);
  },

  async saveModel(model: AIModel): Promise<AIModel> {
    const key = `${model.providerId || 'google'}::${model.id}`;
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_models) jsonState.ai_models = {};
      jsonState.ai_models[key] = model;
      saveJsonState(jsonState);
      return model;
    }
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_models', `${model.providerId || 'google'}__${model.id}`).set(sanitizeForFirestore(model));
    return model;
  },

  async deleteModel(id: string, providerId?: string): Promise<boolean> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_models) return false;
      if (providerId) {
        const key = `${providerId}::${id}`;
        if (jsonState.ai_models[key]) {
          delete jsonState.ai_models[key];
          saveJsonState(jsonState);
          return true;
        }
      }
      if (jsonState.ai_models[id]) {
        delete jsonState.ai_models[id];
        saveJsonState(jsonState);
        return true;
      }
      const entry = Object.entries(jsonState.ai_models).find(([_, m]) => m.id === id && (!providerId || m.providerId === providerId));
      if (entry) {
        delete jsonState.ai_models[entry[0]];
        saveJsonState(jsonState);
        return true;
      }
      return false;
    ...
    const fsdb = getFirestore();
    if (providerId) {
      await docRef(fsdb, 'ai_models', `${providerId}__${id}`).delete();
    } else {
      await docRef(fsdb, 'ai_models', id).delete();
    }
    return true;
  },
  ...
  async getUsages(limitCount: number = 100): Promise<AIUsage[]> {
    if (!USE_FIRESTORE) {
      return Object.values(jsonState.ai_usage || {})
        .sort((a, b) => b.timestamp - a.timestamp)
        .slice(0, limitCount);
    }
    const fsdb = getFirestore();
    const snap = await colRef(fsdb, 'ai_usage').orderBy('timestamp', 'desc').limit(limitCount).get();
    return snap.docs.map((d: any) => ({ ...(d.data() as AIUsage), id: d.id }));
  },

  async saveUsage(usage: AIUsage): Promise<AIUsage> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_usage) jsonState.ai_usage = {};
      jsonState.ai_usage[usage.id] = usage;
      saveJsonState(jsonState);
      return usage;
    }
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_usage', usage.id).set(sanitizeForFirestore(usage));
    return usage;
  },
  ...
  async clearUsages(): Promise<boolean> {
    ...
    const fsdb = getFirestore();
    const snap = await colRef(fsdb, 'ai_usage').get();
    const batch = fsdb.batch();
  ...
  async getHealth(credentialId: string): Promise<AIHealth | null> {
    if (!USE_FIRESTORE) {
      return jsonState.ai_health?.[credentialId] || null;
    }
    const fsdb = getFirestore();
    return getDocData<AIHealth>(fsdb, 'ai_health', credentialId);
  },

  async saveHealth(health: AIHealth): Promise<AIHealth> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_health) jsonState.ai_health = {};
      jsonState.ai_health[health.credentialId] = health;
      saveJsonState(jsonState);
      return health;
    }
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_health', health.credentialId).set(sanitizeForFirestore(health));
    return health;
  },

  async deleteHealth(credentialId: string): Promise<boolean> {
    if (!USE_FIRESTORE) {
      if (jsonState.ai_health?.[credentialId]) {
        delete jsonState.ai_health[credentialId];
        saveJsonState(jsonState);
        return true;
      }
      return false;
    ...
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_health', credentialId).delete();
    return true;
  },

  async getRoutingPolicies(): Promise<AIRoutingPolicy[]> {
    if (!USE_FIRESTORE) {
      return Object.values(jsonState.ai_routing_policies || {});
    }
    const fsdb = getFirestore();
    const snap = await colRef(fsdb, 'ai_routing_policies').get();
    return snap.docs.map((d: any) => ({ ...(d.data() as AIRoutingPolicy), id: d.id }));
  },

  async getRoutingPolicy(id: string): Promise<AIRoutingPolicy | null> {
    if (!USE_FIRESTORE) {
      return jsonState.ai_routing_policies?.[id] || null;
    }
    const fsdb = getFirestore();
    return getDocData<AIRoutingPolicy>(fsdb, 'ai_routing_policies', id);
  },

  async saveRoutingPolicy(policy: AIRoutingPolicy): Promise<AIRoutingPolicy> {
    if (!USE_FIRESTORE) {
      if (!jsonState.ai_routing_policies) jsonState.ai_routing_policies = {};
      jsonState.ai_routing_policies[policy.id] = policy;
      saveJsonState(jsonState);
      return policy;
    }
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_routing_policies', policy.id).set(sanitizeForFirestore(policy));
    return policy;
  },

  async deleteRoutingPolicy(id: string): Promise<boolean> {
    if (!USE_FIRESTORE) {
      if (jsonState.ai_routing_policies?.[id]) {
        delete jsonState.ai_routing_policies[id];
        saveJsonState(jsonState);
        return true;
      }
      return false;
    ...
    const fsdb = getFirestore();
    await docRef(fsdb, 'ai_routing_policies', id).delete();
    return true;
  },
};

// ---------------------------------------------------------------------------
// Database Driver Resolver & Deterministic Selection
// ---------------------------------------------------------------------------
...
export function getDatabaseDriver(): typeof firestoreDb {
  ...
  if (isSupabaseEnabled) {
    if (!isSupabaseConfigured()) {
      throw new Error(
        '[SUPABASE FAIL-CLOSED] SUPABASE_ENABLED=true but missing required configuration (SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY).'
      );
    }
